...
*/
const askUser = async () => {
 const files=fs.readFileSync('./sharing_folder/Flume - Never Be Like You feat. Kai.mp3');
 
 
 for (let id in peers) {
    peers[id].conn.write(files)
  }
  ...
  sw.on('connection', (conn, info) => {
   ...
   console.log(`Connected #${seq} to peer: ${peerId}`)
    ...
    conn.on('data', data => {
      ...
      var wr=fs.writeFileSync('./sharing_folder/myshared.mp3',data);
